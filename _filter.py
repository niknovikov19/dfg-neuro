...
Nikita Novikov:
    For zerophase==True, I use sosfiltfit() instead of explcit double
    filtering with sosfilt(). It allows data to be a multi-dimensional
    array, which will be filtered along the last dimension (the old code
    would filter it along the 0-th dimension)
...
try:
    ...
    from ._sosfilt import _sosfilt as sosfilt, sosfiltfilt
...
def bandpass(data, freqmin, freqmax, df, corners=4, zerophase=True):
    ...
    sos = zpk2sos(z, p, k)
    if zerophase:
        return sosfiltfilt(sos, data, padtype=None)
    else:
