...
def calc_spike_TF_PLV_by_trial_batch(cell_epoched_info, chan_tf_info, ROI_vec, ROIset_name):
# Calculate spike-field coherence for each channel-cell pair in each trial
# Result is calculated for each frequency, in the time intervals given by ROI_vec
# ROI = [{'name': ..., 'tlim': ...}, ...]  
# Output variables:  PLV, Nspikes
# Output dimensions: freq, ROI, trial, cell
    ...
    NROI = len(ROI_vec)
    ROI_names = [ROI['name'] for ROI in ROI_vec]
    ...
    for chan_num in range(Nchan):
        ...
        data_out_w = xr.DataArray(Xnan, coords=coords_w, dims=['freq', 'ROI_num', 'trial_num', 'cell_id'])
        ...
        data_out_N = xr.DataArray(Xnan, coords=coords_N, dims=['ROI_num', 'trial_num', 'cell_id'])
        ...
        postfix = 'spPLV_tr_(%s)' % ROIset_name
    ...
    # Add info about the epoching operation to the output table
    spPLVf_info_out.attrs['ROIset_name'] = ROIset_name
    spPLVf_info_out.attrs['ROI_vec'] = ROI_vec
